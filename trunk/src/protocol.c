...
int post_packet( struct qqclient* qq, qqpacket* p, int key_type )
{
	...
	int head_len = qq->network==TCP ? 13 : 11;
	if( qq->log_packet ){
	...
	switch( key_type )//use this key to enrypt data
	{
		...
		case RANDOM_KEY:
		{
			NEW( encrypted, PACKET_SIZE ,uchar);
			if( !encrypted ) {
			...
			if( p->command == QQ_CMD_LOGIN_SEND_INFO || p->command == QQ_CMD_LOGIN_GET_INFO ||
				p->command == QQ_CMD_LOGIN_GET_LIST ){
...
{
	int out_len = PACKET_SIZE;
	int head_len = qq->network==TCP ? 9 : 7;
	uchar* decrypted;
	NEW( decrypted, PACKET_SIZE ,uchar);
	if( !decrypted ) {
